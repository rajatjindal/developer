...
| `allowed_outbound_hosts` | Optional  | Array of strings | The addresses to which the Wasm component is allowed to send network requests. Each entry must contain both a name (or IP address) and a port in `name:port` format. For Redis, MySQL, and PostgreSQL, the component may only contact the addresses listed here; for HTTP, the component may also contact hosts in `allowed_http_hosts`. If this field is omitted, Spin 1 components default to allow-all access for backward compatibility; otherwise, no outbound access is permitted except for HTTP. | `["example.com:4567", "127.0.0.1:5000"]` |
...
| `variables`             | Optional   | Table       | Dynamic configuration values to be made available to this component. The table keys are user-defined; the values must be strings, and may use template notation as described under [Dynamic Configuration](dynamic-configuration). | `[component.variables]`<br />`api_base_url = "https://{{ api_host }}/v1"` |
